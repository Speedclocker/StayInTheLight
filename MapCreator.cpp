...
void tileset_collision_settings(sf::RenderWindow* window, sf::Texture* texture_file, Tile tiles[], int nbr_avail_tiles, int size_tile)
{
	...
	state_text.loadFromFile("Mapping/state_collision.png");
...
int main(int argc, char* argv[])
{
	...
	if( texture_file.loadFromFile(argv[5]) == false)
	{
	...
	sf::RenderWindow window(sf::VideoMode((size_tile*size_x<1200) ? size_tile*size_x : 1200 , (size_tile*size_x<720) ? size_tile*size_x : 720), "MapCreator");
	...
	Tile available_tiles[nbr_tiles];
	...
	//Création de la map
	Map custom_map(sf::Vector2f(size_x, size_y), size_h, size_tile);
	...
	while(window.isOpen() && tileset_window.isOpen())
	{
		...
		//Choix de la tile
		choice_tile(&buff_tile, &tileset_window, &texture_file, available_tiles, nbr_tiles, size_tile);

		if(tileset_window.hasFocus())
		{
			//Gestion de la collision des blocs
			tileset_collision_settings(&tileset_window, &texture_file, available_tiles, nbr_tiles, size_tile);	
		}
